...
class VatReturnSnapshot(Base):
    ...
    prior_period_correction = Column(Numeric(18, 2), nullable=False, default=0)
    carried_forward_vat = Column(Numeric(18, 2), nullable=False, default=0)
    adjustment_reason = Column(Text)
    adjustments_updated_by = Column(Integer, ForeignKey("users.id"))
    adjustments_updated_at = Column(DateTime)
