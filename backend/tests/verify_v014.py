"""End-to-end verification for CORVAX v0.14 enterprise engines."""
...
os.environ["SECRET_KEY"] = "verification-secret-key-v014"
...
with TestClient(app) as client:
    ...
    # IAS 16 fixed assets: capitalization and 15-day convention depreciation.
    categories = assert_ok(client.get("/api/v1/assets/categories?company_id=1", headers=admin))
    company_bank = assert_ok(client.get("/api/v1/banking/accounts?company_id=1", headers=admin))[0]
    asset = assert_ok(client.post("/api/v1/assets", headers=admin, json={
        "company_id":1,"name_ar":"آلة تعبئة تجريبية","name_en":"Demo Packing Machine","category_id":categories[0]["id"],
        "acquisition_date":"2026-07-12","in_service_date":"2026-07-20","cost":120000,"useful_life_months":60,"bank_account_id":company_bank["id"],
    }), 201)
    assert Decimal(str(asset["net_book_value"])) == Decimal("120000.00")
    depreciation = assert_ok(client.post("/api/v1/assets/depreciation/run", headers=admin, json={"company_id":1,"as_of_date":"2026-07-31"}))
    assert depreciation["posted_count"] == 0 and Decimal(str(depreciation["depreciation_amount"])) == Decimal("0")
    asset_summary = assert_ok(client.get("/api/v1/assets/summary?company_id=1", headers=admin))
    assert Decimal(str(asset_summary["net_book_value"])) == Decimal("120000.00")

    # Payroll: configurable employee rates, accrual, WPS-style payment and accounting journals.
    payroll = assert_ok(client.post("/api/v1/payroll/runs", headers=admin, json={
        "company_id":1,"period_year":2026,"period_month":7,"payment_date":"2026-07-31","bank_account_id":company_bank["id"],
        "adjustments":[{"employee_id":1,"other_deductions":100}],
    }), 201)
    assert payroll["status"] == "CALCULATED" and Decimal(str(payroll["total_net"])) == Decimal("9620.00")
    posted_payroll = assert_ok(client.post(f"/api/v1/payroll/runs/{payroll['id']}/post", headers=admin))
    assert posted_payroll["status"] == "POSTED"
    paid_payroll = assert_ok(client.post(f"/api/v1/payroll/runs/{payroll['id']}/pay", headers=admin))
    assert paid_payroll["status"] == "PAID"

    # Restaurant POS: recipe consumption, VAT, food cost, platform receivable and settlement commission.
    rest_warehouse = assert_ok(client.get("/api/v1/inventory/warehouses?company_id=3", headers=admin))[0]
    rest_bank = assert_ok(client.get("/api/v1/banking/accounts?company_id=3", headers=admin))[0]
    menu = assert_ok(client.get("/api/v1/pos/menu?company_id=3", headers=admin))[0]
    platform = assert_ok(client.get("/api/v1/pos/platforms?company_id=3", headers=admin))[0]
    raw_before = next(r for r in assert_ok(client.get(f"/api/v1/inventory/stock-summary?company_id=3", headers=admin)) if r["item_code"] == "RAW-001")
    pos_order = assert_ok(client.post("/api/v1/pos/orders", headers=admin, json={
        "company_id":3,"order_date":"2026-07-12","warehouse_id":rest_warehouse["id"],"payment_channel":"DELIVERY","platform_id":platform["id"],
        "lines":[{"menu_item_id":menu["id"],"quantity":2}],
    }), 201)
    assert pos_order["status"] == "PENDING_SETTLEMENT"
    assert Decimal(str(pos_order["subtotal"])) == Decimal("70.00") and Decimal(str(pos_order["food_cost"])) == Decimal("9.10")
    settlement = assert_ok(client.post(f"/api/v1/pos/orders/{pos_order['id']}/settle", headers=admin, json={"settlement_date":"2026-07-12","bank_account_id":rest_bank["id"]}))
    assert settlement["status"] == "SETTLED" and Decimal(str(settlement["commission"])) == Decimal("16.91")
    raw_after = next(r for r in assert_ok(client.get(f"/api/v1/inventory/stock-summary?company_id=3", headers=admin)) if r["item_code"] == "RAW-001")
    assert Decimal(str(raw_before["quantity"])) - Decimal(str(raw_after["quantity"])) == Decimal("0.5100")
    pos_summary = assert_ok(client.get("/api/v1/pos/summary?company_id=3", headers=admin))
    assert Decimal(str(pos_summary["gross_profit"])) == Decimal("60.90")
...
print("CORVAX v0.14 enterprise engines: ALL VERIFICATIONS PASSED")
DB_PATH.unlink(missing_ok=True)
